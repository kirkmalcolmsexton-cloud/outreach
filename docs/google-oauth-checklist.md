...
- Alternatively, hydrate that path (and **`MAPS_API_KEY`** in **`local.properties`**, merged from **`development_api_key`** in the team’s consolidated JSON) from the team’s passphrase-encrypted consolidated file using **`android/scripts/setup-secrets.sh`** — see [**README → Firebase config**](../README.md#5-firebase-config-required-for-google-sign-in--firebase).
