...
- **Physical iPhone (CLI):** With the device connected, trusted, **Developer Mode** on, and a signing team that can run the **Outreach** target on that device (same as **Run** in Xcode), use **`bash ios/scripts/deploy-device.sh`**. The script uses **`xcrun devicectl`** (Xcode 15+) to list devices, build with **`OUTREACH_DESTINATION=platform=iOS,id=…`**, install the **`Debug-iphoneos`** app, and launch it. If several devices are attached, pass **`--udid`**, set **`OUTREACH_DEVICE_UDID`**, or **`--pick`**. The repo’s Xcode project keeps **Development Team** empty in git; for command-line device builds, set your Apple team id: **`export OUTREACH_DEVELOPMENT_TEAM=XXXXXXXXXX`** (10‑character Team ID from **developer.apple.com** → **Membership** or **Xcode → target → Signing & Capabilities**), or add it to **`~/etc/outreach.env`**. Do **not** keep a blank line like **`OUTREACH_DEVELOPMENT_TEAM=`** with no value—either omit the variable or set the id ( **`build.sh`** now falls through to **`DEVELOPMENT_TEAM`** when the Outreach variable is unset). **`build.sh`** / **`deploy-device.sh`** source **`~/etc/outreach.env`** when present. If **`xcodebuild`** reports **No Account for Team**, use the same Apple ID under **Xcode → Settings → Accounts** as on the Mac, and try running the script from **Terminal.app** if the IDE terminal cannot sign. See **`bash ios/scripts/deploy-device.sh --help`**. **`OUTREACH_DEPLOY_SKIP_BUILD=1`** reinstalls/launches an existing device build without **`xcodebuild`**.
- **Build (CLI):**  
